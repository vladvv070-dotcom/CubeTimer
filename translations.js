/* ============================================================
   Next Cube Pro -- UI translations (ru/en)
   Extracted from script.js for readability.
   Loaded via <script> before script.js in index.html.
   ============================================================ */

        const TRANSLATIONS = {
            en: {
                // Header
                statistics: 'Statistics',
                sessions: 'Sessions',
                settings: 'Settings',
                more: 'More',
                newScramble: 'New Scramble',
                algsTrainer: 'Algs Trainer',
                multiplayer: 'Multiplayer',
                soon: 'Soon',
                dailyChallengeMenu: 'Daily Scramble',
                dailyChallengeTitle: 'Daily Scramble',
                dailyChallengeLoading: 'Loading today’s scramble…',
                dailyChallengeMissing: 'Today’s scramble has not been published yet. Please check back later.',
                dailyChallengeError: 'Could not load today’s scramble. Check your connection and try again.',
                dailyChallengeSolve: 'Solve this scramble',
                dailyChallengeClose: 'Close',
                dailyChallengeActive: 'Daily Scramble',
                dailyChallengePuzzle: 'Puzzle',
                achievementsMenu: 'Achievements',
                shopMenu: 'Shop',
                streakFrozen: 'Streak freeze used',
                streakOpen: 'Open activity calendar',
                streakTitle: 'Activity',
                streakSubtitle: 'Your solving activity across all sessions',
                streakCurrent: 'Current streak',
                streakBest: 'Best streak',
                streakActiveDays: 'Active days',
                streakDay: 'day',
                streakDays: 'days',
                streakHint: 'Each square is one day',
                streakLess: 'Less',
                streakMore: 'More',
                streakSolves: 'solves',
                streakPreviousMonth: 'Previous month',
                streakNextMonth: 'Next month',
                streakPerfectWeek: 'Perfect week',
                close: 'Close',
                
                // Timer
                timerHint: 'Space to start/stop',
                timerHintMouse: 'Click to start/stop',
                timerHintTouch: 'Tap to start/stop',
                newBest: 'New',
                newBestSingle: 'NEW BEST SINGLE!',
                newBestAo5: 'NEW BEST AO5!',
                newBestAo12: 'NEW BEST AO12!',
                newBestAo100: 'NEW BEST AO100!',
                
                // Controls
                dnf: 'DNF',
                plusTwo: '+2',
                delete: 'Delete',
                edit: 'Edit',
                
                // Left column
                lastSolves: 'Last 5 Solves',
                ao5: 'Ao5:',
                ao12: 'Ao12:',
                
                // Right column
                bestTimes: 'Best Times',
                best: 'Best:',
                sessionAvg: 'Session Avg:',
                averages: 'Averages',
                progress: 'Progress',
                ao100: 'Ao100:',
                
                // Bottom controls
                exportJSON: 'Export JSON',
                exportCSV: 'Export CSV',
                importExport: 'Import / Export',
                importCompleteTitle: '{timer} Import Complete',
                cstImportedLabel: 'imported',
                cstSkippedLabel: 'skipped',
                sessionMarkedAs3x3: '{session} ({puzzle}) is marked as 3x3',
                ieExportTitle: 'EXPORT TO',
                ieImportTitle: 'IMPORT FROM',
                authBtn: 'Log in',
                authRegisterTitle: 'Register',
                authNicknameLabel: 'Nickname',
                authRegEmailLabel: 'Email',
                authRegPasswordLabel: 'Password',
                authRegPasswordRepeatLabel: 'Repeat password',
                authRegisterBtn: 'Register',
                authDividerText: 'or log in',
                authLoginIdLabel: 'Nickname or email',
                authLoginPasswordLabel: 'Password',
                authLoginBtn: 'Log in',
                authCloseBtn: 'Close',
                authWarningTitle: "You're not logged in",
                authWarningText: "You haven't registered or logged in yet. Data is only stored locally in this browser and isn't reliable — you may lose it if the browser cache gets cleared.",
                authWarningCloseBtn: 'Close',
                authErrRequired: 'Please fill in all fields.',
                authErrEmailFormat: 'Enter a valid email address.',
                authErrPasswordShort: 'Password must be at least 6 characters.',
                authErrPasswordMismatch: "Passwords don't match.",
                authErrWrongPassword: 'Incorrect password.',
                authErrUserNotFound: 'No account found with this email.',
                authErrEmailInUse: 'This email is already registered.',
                authErrTooManyRequests: 'Too many attempts. Try again later.',
                authErrGeneric: 'Something went wrong. Please try again.',
                authErrUseGoogle: 'This account signs in with Google. Please use the Google button instead.',
                authAccountTitle: 'Logged in as',
                authAccountCloseBtn: 'Close',
                authLogoutBtn: 'Log out',
                authLogoutConfirm: 'Log out of your account?',
                authLogoutCancelBtn: 'Cancel',
                authChangeNicknameBtn: 'Change nickname',
                authChangeNicknameTitle: 'Change nickname',
                authNewNicknameLabel: 'New nickname',
                authChangeNicknameSaveBtn: 'Save',
                authChangeNicknameCancelBtn: 'Cancel',
                authErrNicknameInUse: 'This nickname is already taken.',
                authChangeNicknameSuccess: 'Nickname updated.',
                tgPromoBtn: 'Check out the latest updates!',
                targetTimeLabel: 'Target Time',
                targetGoalPrefix: 'Goal:',
                secAbbrev: 'sec',
                ttModalTitle: 'Target Time',
                ttEnableLabel: 'Enable',
                ttGoalLabel: 'Goal time',
                honestModeLabel: 'Honest Mode',
                hmModalTitle: 'Honest Mode',
                hmDesc: "While active, solves can't be deleted or edited (+2/DNF still work). Pick how long the honest run should last.",
                hmActiveDesc: "Honest Mode is running. Solves are locked until it ends — it can't be stopped early.",
                hmMinutesLabel: 'Duration (minutes)',
                hmCloseLabel: 'Close',
                honestModeLocked: "This solve is locked by Honest Mode and can't be edited or deleted until it ends.",
                
                // Settings
                settingsTitle: 'Settings',
                sidebarTimer: 'Timer',
                sidebarTheme: 'Theme',
                sidebarSound: 'Sound Effects',
                sidebarLanguage: 'Language',
                sidebarDisplay: 'Display Options',
                sidebarMood: 'Timer Mood',
                sidebarData: 'Data',
                sidebarAnalytics: 'Analytics',
                timerSettingsDelay: 'Timer Settings & Delay',
                interfaceTheme: 'Interface Theme',
                advancedCustomization: 'Advanced Customization',
                lightTheme: 'Light',
                runningTimerColor: 'Running Timer Color',
                timeDisplayFormat: 'Time Display Format',
                enableSoundEffects: 'Enable Sound Effects',
                inspection: 'Inspection',
                inspectionDesc: 'Tap Space to start 15-second inspection before solve',
                inspectionMode: 'Inspection Mode',
                inspectionModeWca: 'DNF after inspection',
                inspectionModeTraining: 'Training',
                inspectionModeTrainingDesc: 'auto-start after inspection',
                voiceInspectionLabel: 'Voice Warnings',
                voiceInspectionDesc: 'Announces "8 seconds" and "12 seconds" during inspection, like a WCA judge',
                voiceInspectionUnsupported: "This browser doesn't support voice synthesis, so Voice Warnings won't be audible here.",
                timerSettings: 'Timer Settings',
                holdDelay: 'Hold delay',
                timerColor: 'Timer Color',
                theme: 'Theme',
                darkTheme: 'Dark Theme',
                audio: 'Audio',
                soundEffects: 'Sound Effects',
                language: 'Language',
                chooseLanguage: 'Choose your interface language',
                displayOptions: 'Display Options',
                timerMode: 'Timer Mode',
                showAo5: 'Show Ao5',
                showAo12: 'Show Ao12',
                showAo100: 'Show Ao100',
                showSessionAvg: 'Show Session Avg',
                timerMood: 'Timer Mood',
                chooseMood: 'Choose how the timer will communicate with you',
                moodFriend: 'Friend',
                moodTeaser: 'Teaser',
                moodTrainer: 'Trainer',
                moodEnemy: 'Enemy',
                moodRival: 'Rival',
                inspection: 'Inspection',
                inspectionDescription: 'Tap Space to start 15-second inspection before solve',
                
                moodSilent: 'Silent',
                commentaryLangLabel: 'Commentary Language',
                dataManagement: 'Data Management',
                resetSession: 'Reset Session',
                resetConfirm: 'Are you sure? This will delete all solves in the current session.',
                
                // Statistics
                statisticsTitle: 'Statistics',
                summary: 'Summary',
                bestSingle: 'Best Single',
                bestAo5: 'Best Ao5',
                bestAo12: 'Best Ao12',
                bestAo100: 'Best Ao100',
                totalSolves: 'Total Solves',
                progressChart: 'Progress Over Time',
                singles: 'Singles',
                chartZoomHint: 'Ctrl+Wheel to zoom · Drag to pan',
                resetZoom: 'Reset Zoom',
                timeDistribution: 'Time Distribution',
                penaltyPieTitle: 'Clean vs Penalties',
                disciplinePieTitle: 'Solves by Puzzle',
                subsessionPieTitle: 'Solves by Subsession',
                pieClean: 'Clean',
                piePlusTwo: '+2',
                pieDnf: 'DNF',
                pieRegularSolves: 'Regular',
                sessionDetails: 'Session Details',
                bestSolve: 'Best Solve',
                worstSolve: 'Worst Solve',
                dnfs: 'DNFs',
                penalties: '+2 Penalties',
                mean: 'Mean',
                stdDev: 'Std Dev',
                averageShort: 'AVG',
                
                // Sessions
                sessionsTitle: 'Sessions',
                newSession: '+ New Session',
                rename: 'Rename',
                noSession: 'No Session',
                defaultSession: 'Default session',
                solves: 'solves',
                solvesCount: '{n} solves',
                deleteSession: 'Delete Session',
                deleteConfirm: 'Delete',
                exportSession: 'Export CSV',

                // Analytics settings
                sidebarAnalyticsLabel: 'Heatmap',
                analyticsTitle: 'Analytics',
                clockFormat: 'Clock Format',
                clockFormat24desc: '13:00',
                clockFormat12desc: '1:00 PM',
                currentTime: 'Current Time',
                timeOffsetHint: 'If your device clock is correct, leave this as-is. The heatmap uses your system time automatically.',
                timeOffsetLabel: 'Time offset (hours)',
                weekStartLabel: 'Streak week starts on',
                weekStartMonday: 'Monday',
                weekStartSunday: 'Sunday',

                // Statistics — Trend
                trendTitle: 'Progress Trend',
                trendStable: 'Stable pace',
                trendFaster: 'faster vs earlier',
                trendSlower: 'slower vs earlier',
                trendNeedMore: 'Need {n} more solves',
                trendFirst: 'First {n} avg',
                trendLast: 'Last {n} avg',

                // Statistics — Heatmap
                heatmapTitle: 'Time of Day Heatmap',
                heatmapHint: 'Shows your average solve time by hour of day',
                heatmapBestHour: 'Best hour',

                // Subsessions
                subsessionsTitle: 'Subsessions',
                subsessionModalTitle: 'New Subsession',
                subsessionNamePlaceholder: 'e.g. Morning practice',
                subsessionColorLabel: 'Color',
                subsessionExcludeLabel: 'Exclude from session average',
                subsessionExcludeHint: "These solves won't affect Ao5, Ao12 and Session Avg",
                subsessionCancel: 'Cancel',
                subsessionCreate: 'Create',
                subsessionExcludedBadge: 'excluded',
                subsessionContextBtn: '＋ Add as Subsession',
                subsessionSolvesSelected: '{n} solves selected',

                // Export Image
                exportImageBtn: '🖼 Export Image',
                exportImageTitle: 'Export Image',
                exportFormatLabel: 'Format',
                exportFormatStory: 'Story',
                exportFormatPost: 'Post',
                exportFormatBanner: 'Wide',
                exportIncludeLabel: 'Include',
                exportOptBest: 'Best Times',
                exportOptTrend: 'Progress Trend',
                exportOptCount: 'Solve Count',
                exportOptDiscipline: 'Discipline',
                exportDownloadBtn: '⬇ Download PNG',
                exportSessionLabel: 'Session',
                cancel: 'Cancel',
                hotkeySessions: 'Sessions',
                hotkeyAchievements: 'Achievements',
                hotkeyShop: 'Shop',
                hotkeySettings: 'Settings',
                hotkeyStats: 'Stats',
                hotkeyDelete: 'Delete',
                hotkeyEdit: 'Edit',

                // Timer behaviour
                hideUiLabel: 'Hide UI During Solve',
                hideUiDesc: 'Hides everything except the timer while solving',
                mouseStartLabel: 'Mouse Start',
                mouseStartDesc: 'Start and stop the timer with mouse click instead of spacebar',
                autoExportLabel: 'Auto-Export',
                autoExportDesc: 'Automatically export your solves every N attempts',
                autoExportEveryLabel: 'Every N solves',
                autoExportFormatLabel: 'Format',
                autoExportFolderDesc: 'Saves straight to a folder, no download prompt',
                autoExportFolderSet: 'Saving to: {folder}',
                autoExportFolderUnset: 'No folder selected — files will go to Downloads',
                autoExportChooseBtn: 'Choose Folder',
                autoExportReverifyBtn: 'Re-verify Access'
            },
            ru: {
                // Header
                statistics: 'Статистика',
                sessions: 'Сессии',
                settings: 'Настройки',
                more: 'Ещё',
                newScramble: 'Новый скрамбл',
                algsTrainer: 'Тренажёр алгоритмов',
                multiplayer: 'Мультиплеер',
                soon: 'Скоро',
                dailyChallengeMenu: 'Скрамбл дня',
                dailyChallengeTitle: 'Скрамбл дня',
                dailyChallengeLoading: 'Загружаем сегодняшний скрамбл…',
                dailyChallengeMissing: 'Скрамбл на сегодня пока не опубликован. Загляните немного позже.',
                dailyChallengeError: 'Не удалось загрузить скрамбл дня. Проверьте интернет и попробуйте снова.',
                dailyChallengeSolve: 'Решить этот скрамбл',
                dailyChallengeClose: 'Закрыть',
                dailyChallengeActive: 'Скрамбл дня',
                dailyChallengePuzzle: 'Головоломка',
                achievementsMenu: 'Достижения',
                shopMenu: 'Магазин',
                streakFrozen: 'Использована заморозка стрика',
                streakOpen: 'Открыть календарь активности',
                streakTitle: 'Активность',
                streakSubtitle: 'Ваши сборки во всех сессиях',
                streakCurrent: 'Текущий стрик',
                streakBest: 'Лучший стрик',
                streakActiveDays: 'Активных дней',
                streakDay: 'день',
                streakDays: 'дней',
                streakHint: 'Каждая клетка — один день',
                streakLess: 'Меньше',
                streakMore: 'Больше',
                streakSolves: 'сборок',
                streakPreviousMonth: 'Предыдущий месяц',
                streakNextMonth: 'Следующий месяц',
                streakPerfectWeek: 'Идеальная неделя',
                close: 'Закрыть',
                
                // Timer
                timerHint: 'Пробел для старта/стопа',
                timerHintMouse: 'Клик для старта/стопа',
                timerHintTouch: 'Тап для старта/стопа',
                newBest: 'Новый',
                newBestSingle: 'НОВЫЙ ЛУЧШИЙ СИНГЛ!',
                newBestAo5: 'НОВЫЙ ЛУЧШИЙ AO5!',
                newBestAo12: 'НОВЫЙ ЛУЧШИЙ AO12!',
                newBestAo100: 'НОВЫЙ ЛУЧШИЙ AO100!',
                
                // Controls
                dnf: 'DNF',
                plusTwo: '+2',
                delete: 'Удалить',
                edit: 'Изменить',
                
                // Left column
                lastSolves: 'Последние 5 сборок',
                ao5: 'Ao5:',
                ao12: 'Ao12:',
                
                // Right column
                bestTimes: 'Лучшие результаты',
                best: 'Лучший:',
                sessionAvg: 'Средний:',
                averages: 'Средние',
                progress: 'Прогресс',
                ao100: 'Ao100:',
                
                // Bottom controls
                exportJSON: 'Экспорт JSON',
                exportCSV: 'Экспорт CSV',
                importExport: 'Импорт / Экспорт',
                importCompleteTitle: 'Импорт из {timer} завершён',
                cstImportedLabel: 'перенесено',
                cstSkippedLabel: 'пропущено',
                sessionMarkedAs3x3: '{session} ({puzzle}) помечена как 3x3',
                ieExportTitle: 'ЭКСПОРТ В',
                ieImportTitle: 'ИМПОРТ ИЗ',
                authBtn: 'Войти',
                authRegisterTitle: 'Регистрация',
                authNicknameLabel: 'Ник',
                authRegEmailLabel: 'Почта',
                authRegPasswordLabel: 'Пароль',
                authRegPasswordRepeatLabel: 'Повторите пароль',
                authRegisterBtn: 'Зарегистрироваться',
                authDividerText: 'или войдите',
                authLoginIdLabel: 'Ник или почта',
                authLoginPasswordLabel: 'Пароль',
                authLoginBtn: 'Войти',
                authCloseBtn: 'Закрыть',
                authWarningTitle: 'Вы не вошли в аккаунт',
                authWarningText: 'Вы ещё не зарегистрировались или не выполнили вход. Данные хранятся только локально в этом браузере и ненадёжны — вы можете периодически терять их при очистке кэша браузера.',
                authWarningCloseBtn: 'Закрыть',
                authErrRequired: 'Заполните все поля.',
                authErrEmailFormat: 'Введите корректный email.',
                authErrPasswordShort: 'Пароль должен быть не короче 6 символов.',
                authErrPasswordMismatch: 'Пароли не совпадают.',
                authErrWrongPassword: 'Неверный пароль.',
                authErrUserNotFound: 'Аккаунт с такой почтой не найден.',
                authErrEmailInUse: 'Эта почта уже зарегистрирована.',
                authErrTooManyRequests: 'Слишком много попыток. Попробуйте позже.',
                authErrGeneric: 'Что-то пошло не так. Попробуйте ещё раз.',
                authErrUseGoogle: 'Этот аккаунт входит через Google. Используйте кнопку Google.',
                authAccountTitle: 'Вы вошли как',
                authAccountCloseBtn: 'Закрыть',
                authLogoutBtn: 'Выйти',
                authLogoutConfirm: 'Выйти из аккаунта?',
                authLogoutCancelBtn: 'Отмена',
                authChangeNicknameBtn: 'Изменить ник',
                authChangeNicknameTitle: 'Изменить ник',
                authNewNicknameLabel: 'Новый ник',
                authChangeNicknameSaveBtn: 'Сохранить',
                authChangeNicknameCancelBtn: 'Отмена',
                authErrNicknameInUse: 'Этот ник уже занят.',
                authChangeNicknameSuccess: 'Ник изменён.',
                tgPromoBtn: 'Узнай о последних обновлениях!',
                targetTimeLabel: 'Цель по времени',
                targetGoalPrefix: 'Цель:',
                secAbbrev: 'сек',
                ttModalTitle: 'Цель по времени',
                ttEnableLabel: 'Включить',
                ttGoalLabel: 'Время цели',
                honestModeLabel: 'Честный режим',
                hmModalTitle: 'Честный режим',
                hmDesc: 'Пока активен, сборки нельзя удалить или отредактировать (+2/DNF по-прежнему работают). Выбери, сколько продлится честный забег.',
                hmActiveDesc: 'Честный режим активен. Сборки заблокированы до его окончания — досрочно остановить нельзя.',
                hmMinutesLabel: 'Длительность (в минутах)',
                hmCloseLabel: 'Закрыть',
                honestModeLocked: 'Эта попытка заблокирована честным режимом — редактирование и удаление станут доступны после его окончания.',
                
                // Settings
                settingsTitle: 'Настройки',
                sidebarTimer: 'Таймер',
                sidebarTheme: 'Тема',
                sidebarSound: 'Звуки',
                sidebarLanguage: 'Язык',
                sidebarDisplay: 'Отображение',
                sidebarMood: 'Настроение',
                sidebarData: 'Данные',
                sidebarAnalytics: 'Аналитика',
                timerSettingsDelay: 'Настройки таймера и задержка',
                interfaceTheme: 'Тема интерфейса',
                advancedCustomization: 'Расширенная кастомизация',
                lightTheme: 'Светлая',
                runningTimerColor: 'Цвет работающего таймера',
                timeDisplayFormat: 'Формат отображения времени',
                enableSoundEffects: 'Включить звуковые эффекты',
                inspection: 'Инспекция',
                inspectionDesc: 'Нажмите пробел для запуска 15-секундной инспекции перед сборкой',
                timerSettings: 'Настройки таймера',
                holdDelay: 'Задержка удержания',
                timerColor: 'Цвет таймера',
                theme: 'Тема',
                darkTheme: 'Тёмная тема',
                audio: 'Аудио',
                soundEffects: 'Звуковые эффекты',
                language: 'Язык',
                chooseLanguage: 'Выберите язык интерфейса',
                displayOptions: 'Параметры отображения',
                timerMode: 'Режим таймера',
                showAo5: 'Показать Ao5',
                showAo12: 'Показать Ao12',
                showAo100: 'Показать Ao100',
                showSessionAvg: 'Показать средний',
                timerMood: 'Настроение таймера',
                chooseMood: 'Выберите, как таймер будет с вами общаться',
                moodFriend: 'Друг',
                moodTeaser: 'Подкалыватель',
                moodTrainer: 'Тренер',
                moodEnemy: 'Враг',
                moodRival: 'Соперник',
                inspection: 'Инспекция',
                inspectionDescription: 'Нажмите пробел для запуска 15-секундной инспекции перед сборкой',
                inspectionMode: 'Режим осмотра',
                inspectionModeWca: 'DNF после осмотра',
                inspectionModeTraining: 'Тренировка',
                inspectionModeTrainingDesc: 'автостарт после осмотра',
                voiceInspectionLabel: 'Голосовые предупреждения',
                voiceInspectionDesc: 'Объявляет "8 секунд" и "12 секунд" во время осмотра, как судья на соревнованиях WCA',
                voiceInspectionUnsupported: 'Этот браузер не поддерживает синтез речи, поэтому голосовые предупреждения тут не будут слышны.',
                moodSilent: 'Молчун',
                commentaryLangLabel: 'Язык комментариев',
                dataManagement: 'Управление данными',
                resetSession: 'Сбросить сессию',
                resetConfirm: 'Вы уверены? Это удалит все сборки текущей сессии.',
                
                // Statistics
                statisticsTitle: 'Статистика',
                summary: 'Сводка',
                bestSingle: 'Лучший',
                bestAo5: 'Лучший Ao5',
                bestAo12: 'Лучший Ao12',
                bestAo100: 'Лучший Ao100',
                totalSolves: 'Всего сборок',
                progressChart: 'Прогресс со временем',
                singles: 'Сборки',
                chartZoomHint: 'Ctrl+колесо — масштаб · Перетаскивание — обзор',
                resetZoom: 'Сбросить масштаб',
                timeDistribution: 'Распределение времени',
                penaltyPieTitle: 'Чисто и штрафы',
                disciplinePieTitle: 'Сборки по головоломкам',
                subsessionPieTitle: 'Сборки по подсессиям',
                pieClean: 'Чисто',
                piePlusTwo: '+2',
                pieDnf: 'DNF',
                pieRegularSolves: 'Обычные',
                sessionDetails: 'Детали сессии',
                bestSolve: 'Лучшая сборка',
                worstSolve: 'Худшая сборка',
                dnfs: 'DNF',
                penalties: 'Штрафы +2',
                mean: 'Среднее',
                stdDev: 'Стд. откл.',
                averageShort: 'СРЕД.',
                
                // Sessions
                sessionsTitle: 'Сессии',
                newSession: '+ Новая сессия',
                rename: 'Переименовать',
                noSession: 'Без сессии',
                defaultSession: 'Сессия по умолчанию',
                solves: 'сборок',
                solvesCount: 'Сборок: {n}',
                deleteSession: 'Удалить сессию',
                deleteConfirm: 'Удалить',
                exportSession: 'Экспорт CSV',

                // Analytics settings
                sidebarAnalyticsLabel: 'Тепловая карта',
                analyticsTitle: 'Аналитика',
                clockFormat: 'Формат времени',
                clockFormat24desc: '13:00',
                clockFormat12desc: '1:00 PM',
                currentTime: 'Текущее время',
                timeOffsetHint: 'Если часы устройства правильные, оставьте как есть. Тепловая карта использует системное время автоматически.',
                timeOffsetLabel: 'Смещение времени (часы)',
                weekStartLabel: 'Начало недели ударного режима',
                weekStartMonday: 'Понедельник',
                weekStartSunday: 'Воскресенье',

                // Statistics — Trend
                trendTitle: 'Прогресс',
                trendStable: 'Стабильный темп',
                trendFaster: 'быстрее чем первые',
                trendSlower: 'медленнее чем первые',
                trendNeedMore: 'Ещё {n} сборок',
                trendFirst: 'Первые {n} avg',
                trendLast: 'Последние {n} avg',

                // Statistics — Heatmap
                heatmapTitle: 'Тепловая карта по часам',
                heatmapHint: 'Среднее время сборки по часам суток',
                heatmapBestHour: 'Лучший час',

                // Subsessions
                subsessionsTitle: 'Под-сессии',
                subsessionModalTitle: 'Новая под-сессия',
                subsessionNamePlaceholder: 'Например: Утренняя тренировка',
                subsessionColorLabel: 'Цвет',
                subsessionExcludeLabel: 'Исключить из среднего сессии',
                subsessionExcludeHint: 'Эти сборки не повлияют на Ao5, Ao12 и общее среднее',
                subsessionCancel: 'Отмена',
                subsessionCreate: 'Создать',
                subsessionExcludedBadge: 'исключено',
                subsessionContextBtn: '＋ Добавить как под-сессию',
                subsessionSolvesSelected: 'Выбрано {n} сборок',

                // Export Image
                exportImageBtn: '🖼 Экспорт изображения',
                exportImageTitle: 'Экспорт изображения',
                exportFormatLabel: 'Формат',
                exportFormatStory: 'История',
                exportFormatPost: 'Пост',
                exportFormatBanner: 'Баннер',
                exportIncludeLabel: 'Включить',
                exportOptBest: 'Лучшие результаты',
                exportOptTrend: 'Прогресс',
                exportOptCount: 'Кол-во сборок',
                exportOptDiscipline: 'Дисциплина',
                exportDownloadBtn: '⬇ Скачать PNG',
                exportSessionLabel: 'Сессия',
                cancel: 'Отмена',
                hotkeySessions: 'Сессии',
                hotkeyAchievements: 'Достижения',
                hotkeyShop: 'Магазин',
                hotkeySettings: 'Настройки',
                hotkeyStats: 'Статистика',
                hotkeyDelete: 'Удалить',
                hotkeyEdit: 'Изменить',

                // Timer behaviour
                hideUiLabel: 'Скрывать UI во время сборки',
                hideUiDesc: 'Скрывает всё кроме таймера пока идёт сборка',
                mouseStartLabel: 'Запуск мышкой',
                mouseStartDesc: 'Запускать и останавливать таймер кликом мыши вместо пробела',
                autoExportLabel: 'Автоэкспорт',
                autoExportDesc: 'Автоматически экспортировать сборки каждые N попыток',
                autoExportEveryLabel: 'Каждые N сборок',
                autoExportFormatLabel: 'Формат',
                autoExportFolderDesc: 'Сохраняет прямо в папку, без диалога скачивания',
                autoExportFolderSet: 'Сохраняется в: {folder}',
                autoExportFolderUnset: 'Папка не выбрана — файлы будут уходить в Загрузки',
                autoExportChooseBtn: 'Выбрать папку',
                autoExportReverifyBtn: 'Обновить доступ'
            }
        };

window.translations = TRANSLATIONS;
